  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Pharmacy Inventory</h2>
          <p className="text-xs text-slate-500">View stocks of homeopathy dilutions, pills, mother tinctures, and ointments.</p>
        </div>
      </div>

      {editingItem && (
        <Card title={`Update Stock: ${editingItem.name}`}>
      ...
      <Table headers={['Item ID', 'Medicine Name', 'Category', 'Stock Level', 'Unit Price', 'Status', 'Actions']}>
        {items.map((item) => (
          <tr key={item.id} className="hover:bg-slate-50/50">
            ...
            <td className="px-4 py-3 text-slate-800 font-medium">{item.stock} {item.unit}</td>
            <td className="px-4 py-3 font-semibold text-slate-800">${item.price.toFixed(2)}</td>
            <td className="px-4 py-3">
              <Badge variant={
